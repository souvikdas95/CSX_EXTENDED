...
void Client_ResetHUD(void* mValue){
	...
	static float handle_flood[33];

	if ( !isModuleActive()	\
		|| !mPlayer	\
		|| !mPlayer->rank	\
		|| MODE_FULLUPDATE[mPlayerIndex]	\
		|| gpGlobals->time - handle_flood[mPlayerIndex] < 1.1 /*tested < 1.001*/ ){
		return;
	}

	mPlayer->rank->updatePosition( &mPlayer->life );
	mPlayer->restartStats(false);
	handle_flood[mPlayerIndex] = gpGlobals->time;
}
...
void Client_DeathMsg(void *mValue)
{
	...
	static int killer_hs;
	static int killer_weapon;
	static int killer_tk;
		...
		case 3:
			{
				...
				// So, it can detect deaths more precisely
				if ( !victim_id )
					break;
				...
				if ( killer_id && killer_id != victim_id )
				{
					...
					if ( !strcmp(tw_name, "knife") )
						killer->aiming = is_headshot ? 1 : 0;

					killer_weapon = killer->current;
					killer_hs = ( killer->aiming == 1 ) ? 1 : 0;
					killer_tk = ( killer->teamId == victim->teamId ) ? 1 : 0;

					if ( killer_weapon != CSW_C4 )
...
void Client_Damage_End(void* mValue){
	...
	static int TA; // why make it global?

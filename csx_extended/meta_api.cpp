...
void ClientKill(edict_t *pEntity) { ClientKill_wasAlive = IsAlive(pEntity); }

void ClientKill_Post(edict_t *pEntity)
{
	if(!ClientKill_wasAlive || IsAlive(pEntity))
...
void ServerActivate_Post(edict_t *pEdictList, int edictCount, int clientMax)
{
	...
	{
#ifdef _DEBUG
		MF_SyncLog("ServerActivate_Post: Calling 'MakeHookSpawn_Player'");
#endif
...
void ServerDeactivate()
{
	...
	if(csstats_reset->value != 0.0	\
		|| csstats_maxsize->value == 0.0	\
		|| (csstats_maxsize->value > 0.0 && g_rank.getRankNum() > (uint32_t)csstats_maxsize->value))
	{
...
void ClientUserInfoChanged_Post(edict_t *pEntity, char *infobuffer)
{
	...
	if(pPlayer->rank != nullptr)
	{
		...
		{
			if((uint8_t)csstats_rank->value == 0	\
				|| ((uint8_t)csstats_rank->value == 1 && (pPlayer->authid == nullptr || !IsValidAuth(pPlayer->authid)))	\
				|| ((uint8_t)csstats_rank->value == 2 && pPlayer->ip == nullptr))
			{
...
#ifdef _DEBUG
		MF_SyncLog("SetClientKeyValue: Calling 'MakeHookSpawn_CSBot'");
#endif
...
void OnMetaAttach()
{
	csstats_maxsize = new cvar_t{ "csstats_maxsize","-1" };
	csstats_reset = new cvar_t{ "csstats_reset","0" };
	csstats_rank = new cvar_t{ "csstats_rank","1" };
	csstats_rankbots = new cvar_t{ "csstats_rankbots","0" };
	csstats_pause = new cvar_t{ "csstats_pause","0" };

	CVAR_REGISTER(csstats_maxsize);
	CVAR_REGISTER(csstats_reset);
	CVAR_REGISTER(csstats_rank);
	CVAR_REGISTER(csstats_rankbots);
	CVAR_REGISTER(csstats_pause);

	csstats_maxsize = CVAR_GET_POINTER(csstats_maxsize->name);
	csstats_reset = CVAR_GET_POINTER(csstats_reset->name);
	csstats_rank = CVAR_GET_POINTER(csstats_rank->name);
	csstats_rankbots = CVAR_GET_POINTER(csstats_rankbots->name);
	csstats_pause = CVAR_GET_POINTER(csstats_pause->name);
}

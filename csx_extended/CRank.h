...
class RankSystem
{
	...
	class RankStats : public Stats
	{
		...
		inline const char* getUnique() const
		{
		...
		inline void updatePosition(Stats* points = nullptr, bool sync = false)
		{
			parent->updatePos_init(this, points, sync);
		}
	...
	void updatePos_init(RankStats* r, Stats* s = nullptr, bool sync = false);
	...
	inline uint32_t getRankNum() const
	{
		return rankNum;
	}
	void clear();
	inline void unloadCalc()
	{
	...
	inline iterator itr_head()
	{
		return iterator(head);
	}
	inline iterator itr_tail()
	{
